...
She's going to build it with `hylomorph`, a spec-as-source agent loop. The harness drives a coding agent (Claude) to implement code task-by-task, but it does something the usual agent workflow doesn't: it treats the spec as the source of truth and the code as a rendering. The agent never edits the spec. Humans never hand-edit the generated code. And if the spec changes, the code gets regenerated — not patched.
...
This scaffolds `.hylomorph/` (config, guardrails, prompts) and `evals/` (where correctness oracles live), and installs a git pre-commit hook that will start checking for drift once she's got specs in place.
...
```sh
hylomorph spec new tido --brief "A CLI task manager: add, complete, and report tasks. Persist to ~/.tido/tasks.json."
```
...
First, **protected write enforcement**: the harness checks the git diff and verifies the agent didn't touch anything outside `src/`. The spec files under `.specs/`, the eval scripts under `evals/`, and the harness config under `.hylomorph/` are all protected. If the agent had tried to modify `1-requirements.json` — perhaps trying to "fix" an ambiguous requirement by changing the spec rather than asking for clarification — the iteration would fail and the working tree would reset. The spec is the source of truth; the agent is not allowed to rewrite it.

Second, **gate validation**: the harness runs the configured gates — `run_build`, `run_lint`, `run_unit_tests`. If any fail, the iteration is marked as failed, the working tree resets, and the task goes back to Todo for another attempt. The failing gate's output is captured on the task, and on the next attempt it's fed back into the prompt as a "Previous attempt failed — fix this first" section — so the agent debugs the actual error instead of blindly re-running the same approach. (Want to see exactly what the agent will receive? `hylomorph explain T-001`.)
...
```json
// .hylomorph/manifest.json (excerpt)
{
  "specs": {
    "tido": {
...
When `evals/tido/` exists, `hylomorph check` enforces coverage:

```sh
hylomorph check tido
...
```sh
hylomorph check tido
✗ tido: spec changed, code not rebuilt — run `hylomorph rebuild tido`
```
...
```sh
hylomorph check tido --reverse
```

The harness drives an agent to read everything under `src/` and reconstruct a spec in Markdown — without being allowed to look at `.specs/tido/`. It writes the result to `.hylomorph/roundtrip/tido.reconstructed.md`.
...
Verdict: DRIFT/UNDERSPECIFIED
  ...
  Hidden behavior — reconstruction describes things the spec doesn't:
    Review .hylomorph/roundtrip/tido.reconstructed.md for undocumented behavior.
```
...
`--reverse` is advisory: Maya reviews the reconstruction herself, decides the `--quiet` flag is worth keeping, and adds it to the spec. `REQ-005` is a legit gap — she re-runs `hylomorph rebuild tido` to bring the code up to the spec.
...
**Cross-model review**: when `hylomorph rebuild tido` runs, after the evals pass, it submits the regenerated code to a second, independent model configured as the reviewer. The reviewer reads the requirements and the new code and checks them against each other — not against the old implementation. If the reviewer finds a requirement that the code doesn't satisfy, the regeneration is rejected and rolled back.
...
```sh
hylomorph check tido --determinism
```
...
When requirements change — and they will — the workflow is: edit the spec, run `hylomorph rebuild`, review the diff. Not: find every place the old behavior was hardcoded, patch them, hope you didn't miss one.
...
|---|---|
| `hylomorph init` | Scaffold `.hylomorph/`, `evals/`, pre-commit gate |
| `hylomorph spec requirements <name> --brief "..."` | Layer 1: draft requirements from a brief |
| `hylomorph spec design <name>` | Layer 2: draft design (gated on requirements) |
| `hylomorph spec tasks <name>` | Layer 3: draft tasks (gated on design) |
| `hylomorph spec new <name> --brief "..."` | Run layers 1→3 in order, each gated |
| `hylomorph spec status <name>` | Show the five-layer ladder + next allowed action |
| `hylomorph check <name>` | Spec well-formed + eval coverage + drift |
| `hylomorph build <name>` | Incremental render loop (spec-anchored) |
| `hylomorph check --all` | Detect drift; exits 2 on any |
| `hylomorph check <name> --accept` | Snapshot current code as the baseline |
| `hylomorph rebuild <name>` | Burn owned files, re-render from spec, gate on evals |
| `hylomorph rebuild <name> --only "src/storage/**"` | Rebuild a subset of owned files |
| `hylomorph check <name> --determinism` | Determinism probe: rebuild N times (`--passes N`, default 2) and report artifact-convergence score |
| `hylomorph check <name> --reverse` | Reconstruct spec from code, emit convergence verdict |
| `hylomorph eval draft <name>` | Draft reviewable eval stubs from acceptance criteria (independent reviewer model) |
| `hylomorph eval run <name>` | Run the oracle against current code |
| `hylomorph gate check` | Verify every referenced gate exists and is executable |
| `hylomorph explain <task>` | Preview the exact prompt the agent would receive (no run) |
| `hylomorph log --follow` | Stream a one-line summary of each iteration as it completes |
| `hylomorph doctor` | Environment/config health check |
...
**Drift classes reported by `hylomorph check`:**

| Class | Cause | Fix |
|---|---|---|
| `Unrecorded` | Spec has `owns` but no baseline was written | `hylomorph rebuild <spec>` or `hylomorph check <spec> --accept` |
| `StaleCode` | Spec inputs changed since last record | `hylomorph rebuild <spec>` |
| `CodeDrift` | Owned file hash changed (hand-edit) | Revert the edit; code is not source |
| `Missing` | An owned file was deleted outside of rebuild | `hylomorph rebuild <spec>` |
