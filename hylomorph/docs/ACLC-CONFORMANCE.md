# ACLC v0.1 Conformance — Hylomorph
...
| Concern | Location |
|---|---|
| Config model, validator, presets, JSON Schema | `crates/hylomorph-core/src/aclc.rs` |
| `LEARNINGS.md` memory (§7.2–7.4) | `crates/hylomorph-core/src/memory.rs` |
| Protected oracle + partial score (§8.2/§8.4) | `crates/hylomorph-core/src/oracle.rs` |
| Lifecycle, workspace, exhaustion, reflection | `crates/hylomorph-core/src/loop_runner.rs` |
| Config table + legacy reconciliation + writer | `crates/hylomorph-core/src/config.rs` |
| CLI surface (`hylomorph aclc …`, doctor) | `crates/hylomorph-cli/src/main.rs` |
| GUI settings panel | `crates/hylomorph-gui/src/main.rs` |

## Configuration

ACLC is the `[aclc]` table in `.hylomorph/hylomorph.toml`:
...
- **§7.3 learning** — `raw` (verbatim failure signal, truncated) and `reflection`
  ...
  `.hylomorph/prompts/reflect.md` or a built-in default).
...
- Reflection and compaction prompts: `.hylomorph/prompts/reflect.md` (built-in
  default provided); compaction is deterministic recency+dedup.

## CLI

```
hylomorph aclc show         # resolved config + matching preset
hylomorph aclc validate     # findings; exit 1 on any error
hylomorph aclc preset <name> [--oracle "<cmd>"]
hylomorph aclc schema       # write .hylomorph/schema/aclc-0.1.schema.json
hylomorph doctor            # includes aclc validity + warnings
```
