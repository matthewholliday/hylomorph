...
pub struct Task {
    ...
    /// `[loop].phase_sequence` from hylomorph.toml for this task only.
    #[serde(default)]
...
pub struct RequirementsFile {
    ...
    /// and the only files that are deleted/rebuilt during `hylomorph regen`.
    #[serde(default)]
    pub owns: Vec<String>,
    /// Stability tier: "weekly" | "monthly" | "yearly" | "never".
    /// `hylomorph regen` refuses to burn a "never" layer without --force-boundary.
    #[serde(default)]
