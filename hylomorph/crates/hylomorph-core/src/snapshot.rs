...
//! (`.hylomorph/logs/state.json`, `.hylomorph/logs/iterations/*.json`,
//! `.hylomorph/logs/progress.md`, and the per-spec `3-tasks.jsonl`). A [`Snapshot`]
...
//! finished run. Both the terminal dashboard (`hylomorph watch`) and the desktop
//! GUI render from this same type.

use std::path::{Path, PathBuf};
use std::time::{Duration, SystemTime};

use crate::config::{load_guardrails, load_hylomorph_config};
...
fn iterations_dir(root: &Path) -> PathBuf {
    root.join(".hylomorph").join("logs").join("iterations")
}
...
fn load_progress_tail(root: &Path, n: usize) -> Vec<String> {
    let path = root.join(".hylomorph").join("logs").join("progress.md");
...
fn newest_mtime(root: &Path) -> Option<SystemTime> {
    ...
    consider(root.join(".hylomorph").join("logs").join("state.json"));
    consider(root.join(".hylomorph").join("logs").join("progress.md"));
