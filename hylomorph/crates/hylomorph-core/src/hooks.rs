...
/// Resolve a hook name to an executable script in `.hylomorph/scripts/hooks/`.
...
pub fn run_hook(
    ...
    cmd.current_dir(root)
        .env("HYLOMORPH_HOOK", &inv.hook_name)
        .env("HYLOMORPH_TASK_ID", &inv.task_id)
        .env("HYLOMORPH_SPEC", &inv.spec_name)
        .env("HYLOMORPH_ITERATION", inv.iteration.to_string())
        .env("HYLOMORPH_ATTEMPT", inv.attempt.to_string())
...
/// Persist full hook output to `.hylomorph/logs/hooks/<ts>-<hook>.log`; returns the path.
pub fn save_hook_log(
    ...
    let dir = root.join(".hylomorph").join("logs").join("hooks");
