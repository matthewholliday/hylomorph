...
fn default_commit_message_template() -> String {
    "hylomorph: complete task {task_id} ({task_title})".to_string()
}
...
fn default_loop_prompt() -> String {
    ".hylomorph/prompts/loop.md".to_string()
}

fn default_init_prompt() -> String {
    ".hylomorph/prompts/init.md".to_string()
}
...
pub struct HylomorphConfig {
    ...
    /// fields fall back to legacy `[loop]` aliases — see [`HylomorphConfig::aclc`]
    /// vs. [`resolve_aclc`].
    #[serde(default)]
    pub aclc: AclcConfig,
...
pub fn resolve_aclc(cfg: &HylomorphConfig, guardrails: &GuardrailsConfig) -> AclcConfig {
    if cfg.aclc_present {
...
/// `.specs/**`, `evals/**`, `.hylomorph/guardrails/**`, `.hylomorph/manifest.json`,
...
pub fn load_hylomorph_config(root: &Path) -> Result<HylomorphConfig> {
    let path = root.join(".hylomorph").join("hylomorph.toml");

    if !path.exists() {
        return Ok(HylomorphConfig::default());
    }

    let content = std::fs::read_to_string(&path)
        .with_context(|| format!("failed to read {}", path.display()))?;

    let raw: RawHylomorphConfig =
...
/// Write the `[aclc]` table into hylomorph.toml, preserving every other key,
/// table, and comment. Creates the file (and `.hylomorph/`) if absent. Inert
...
pub fn save_aclc_config(root: &Path, aclc: &AclcConfig) -> Result<()> {
    ...
    let t = doc["aclc"]
        .as_table_mut()
        .context("`aclc` in hylomorph.toml is not a table")?;
...
pub fn find_project_root() -> Result<PathBuf> {
    ...
    loop {
        ...
        match dir.parent() {
            ...
            None => {
                anyhow::bail!(
                    "could not find .hylomorph/ directory — run `hylomorph init` first or cd into a harness project"
                );
            }
        }
    }
...
mod tests {
    ...
    fn save_budgets_preserves_other_keys_and_comments() {
        let dir = std::env::temp_dir().join(format!("hylomorph-cfg-keep-{}", std::process::id()));
        let gdir = dir.join(".hylomorph").join("guardrails");
    ...
    /// preserves unrelated tables/comments in hylomorph.toml.
    #[test]
    fn save_aclc_round_trips_and_preserves() {
        use crate::aclc::Preset;
        let dir = std::env::temp_dir().join(format!("hylomorph-aclc-rt-{}", std::process::id()));
        std::fs::remove_dir_all(&dir).ok();
        std::fs::create_dir_all(dir.join(".hylomorph")).unwrap();
        std::fs::write(
            dir.join(".hylomorph").join("hylomorph.toml"),
        ...
        let raw = std::fs::read_to_string(dir.join(".hylomorph").join("hylomorph.toml")).unwrap();
        ...
        let cfg = load_hylomorph_config(&dir).unwrap();
    ...
    fn save_budgets_creates_file_when_absent() {
        let dir = std::env::temp_dir().join(format!("hylomorph-cfg-new-{}", std::process::id()));
        std::fs::remove_dir_all(&dir).ok();
