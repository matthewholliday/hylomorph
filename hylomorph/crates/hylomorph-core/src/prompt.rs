...
use crate::config::HylomorphConfig;
...
fn load_template(
    ...
    let prompts_dir = root.join(".hylomorph").join("prompts");
...
pub fn compose_prompt(
    ...
    let rules_path = root.join(".hylomorph").join("guardrails").join("rules.md");
    ...
    let footer = format!(
        "{retry_section}\n\n## Your task\nID: {}{}\nTitle: {}\nAcceptance:\n{}\n\nDo ONLY this task. Leave the project buildable. Update .hylomorph/logs/progress.md with what you did. Then stop.\n\n**Do not modify any file under `.specs/` or `.hylomorph/` (other than `.hylomorph/logs/progress.md`). The harness will fail this iteration if you do.**",
        task.id,
        phase_header,
        task.title,
        task_acceptance,
...
pub fn write_prompt_file(prompt: &str) -> Result<(PathBuf, String)> {
    ...
    let tmp_path = std::env::temp_dir().join(format!("hylomorph-prompt-{}.md", &hash[..16]));
