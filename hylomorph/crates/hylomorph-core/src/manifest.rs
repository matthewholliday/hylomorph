...
pub fn manifest_path(root: &Path) -> PathBuf {
    root.join(".hylomorph").join("manifest.json")
}
...
/// Called after a successful run iteration or `hylomorph manifest record`.
pub fn record_spec(root: &Path, spec_name: &str) -> Result<()> {
    ...
    let model_id = crate::config::load_hylomorph_config(root)
...
pub fn model_drift(root: &Path, spec_name: &str) -> Result<Option<(String, String)>> {
    ...
    let current = crate::config::load_hylomorph_config(root)
        .map(|c| crate::config::model_id_from_command(&c.agent.command))
        .unwrap_or_default();
