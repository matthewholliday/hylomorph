...
impl Layer {
    ...
    pub fn produce_cmd(self, spec: &str) -> String {
        match self {
            Layer::Requirements => format!("hylomorph spec requirements {spec} --brief \"…\""),
            Layer::Design => format!("hylomorph spec design {spec}"),
            Layer::Tasks => format!("hylomorph spec tasks {spec}"),
            Layer::Code => format!("hylomorph build {spec}"),
            Layer::Evals => format!("hylomorph eval draft {spec}"),
        }
    }
...
/// graph integrity — lives in `hylomorph check`.
        ...
        match state.status(layer) {
            ...
            LayerStatus::Invalid(why) => anyhow::bail!(
                "cannot {action}: the '{}' layer is present but invalid: {why}\n  \
                 fix it first:  hylomorph check {spec}",
                layer.label(),
            ),
        }
    }
...
mod tests {
    ...
    fn tmp_root() -> PathBuf {
        ...
        let dir =
            std::env::temp_dir().join(format!("hylomorph-layers-test-{}-{n}", std::process::id()));
