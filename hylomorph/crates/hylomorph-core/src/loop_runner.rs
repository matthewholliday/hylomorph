...
use crate::config::{load_guardrails, load_hylomorph_config, resolve_aclc, PhaseConfig};
use crate::hooks::{
...
/// `.hylomorph/prompts/reflect.md`. The `{...}` placeholders are filled in by
/// [`derive_reflection`].
...
/// application-defined (§10): `.hylomorph/prompts/reflect.md` if present, else the
/// built-in default. Errors propagate so the caller can fall back to `raw`.
fn derive_reflection(
    root: &Path,
    config: &crate::config::HylomorphConfig,
    working_dir: &str,
    task: &Task,
    failure_signal: &str,
) -> Result<String> {
    let custom = root.join(".hylomorph").join("prompts").join("reflect.md");
...
///   - `.hylomorph/` except `.hylomorph/logs/`
///   - `.hylomorph/manifest.json`
///   - `.git/`
...
fn check_protected_writes(
    ...
    fn is_protected(path: &str, deny_set: Option<&GlobSet>, extra: Option<&GlobSet>) -> bool {
        ...
        if path == ".hylomorph/manifest.json" {
            return true;
        }
        if path.starts_with(".hylomorph/") && !path.starts_with(".hylomorph/logs/") {
            return true;
        }
