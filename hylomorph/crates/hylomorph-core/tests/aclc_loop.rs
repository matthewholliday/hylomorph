...
/// agent and oracle. `aclc_toml` is spliced into hylomorph.toml. Returns the root.
fn scaffold(name: &str, agent_sh: &str, oracle_cmd: &str, aclc_toml: &str) -> std::path::PathBuf {
    ...
    // hylomorph.toml: agent runs our script; ACLC block from the caller.
    write(
    ...
    write(
        &root.join(".hylomorph/prompts/loop.md"),
        "Do task {task_id}.\n",
    );
    ...
    // Mirror real projects: spec/hylomorph runtime state is gitignored, so the
...
fn until_pass_loops_then_passes_and_records_memory() {
    ...
    let learnings = root.join(".hylomorph/logs/learnings/demo/T-1.md");
