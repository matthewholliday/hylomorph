...
use hylomorph_core::config::{find_project_root, load_hylomorph_config};
use hylomorph_core::hooks::{run_hook, HookInvocation};
use hylomorph_core::layers::{layer_state, require, Layer};
use hylomorph_core::loop_runner::{run, RunOptions};
use hylomorph_core::manifest::{check_spec, record_spec, DriftKind};
use hylomorph_core::prompt::compose_prompt;
use hylomorph_core::scope::{dirty_paths, enforce, WriteScope};
use hylomorph_core::spec::{
    list_specs, load_requirements, load_tasks, save_tasks, spec_dir, Task, TaskStatus,
};
use hylomorph_core::state::load_state;
...
#[derive(Subcommand)]
enum Commands {
    /// Scaffold .hylomorph/ in the current directory.
    Init {
        #[arg(long)]
        force: bool,
    },
...
enum AclcCmd {
    ...
    Validate,
    /// Apply a named preset to [aclc] in hylomorph.toml
    ...
    /// Write the canonical ACLC JSON Schema to .hylomorph/schema/.
    Schema,
}
...
fn deprecate(old: &str, new: &str) {
    eprintln!("note: '{old}' is deprecated — use '{new}' (see `hylomorph --help`)");
}
...
fn print_drift(name: &str, drift: &DriftKind) {
    match drift {
        DriftKind::Unrecorded { .. } => println!(
            "✗ {name}: no baseline — run `hylomorph rebuild {name}` or `hylomorph check {name} --accept`"
        ),
        DriftKind::StaleCode { .. } => {
            println!("✗ {name}: spec changed, code not rebuilt — run `hylomorph rebuild {name}`")
        }
        DriftKind::CodeDrift { path } => {
            println!("✗ {name}: hand-edit detected in owned file: {path}")
        }
        DriftKind::Missing { path } => {
            println!("✗ {name}: owned file missing: {path} — run `hylomorph rebuild {name}`")
        }
    }
}
...
fn cmd_check(
    ...
    for name in &specs {
        ...
        if let Ok(Some((recorded, current))) = hylomorph_core::manifest::model_drift(root, name) {
            println!(
...
fn cmd_init(root: &Path, _from_specs: bool, force: bool) -> Result<()> {
    let harness = root.join(".hylomorph");
    let scaffold: &[(&str, &str)] = &[
        ("hylomorph.toml", HYLOMORPH_TOML),
    ...
    // Phase 0: install a git pre-commit hook that runs `hylomorph check --all`.
    ...
    let run_gui = root.join("run-hylomorph-gui.sh");
    ...
    println!("\nInitialized hylomorph at {}", harness.display());
    println!(
        "\nOptional: in Claude Code, run the `hylomorph-setup` agent to wire the\n\
...
fn run_layer_agent(root: &Path, layer: Layer, prompt: &str, scope_globs: &[String]) -> Result<()> {
    ...
    let config = load_hylomorph_config(root)?;
    let cmd_str = config
        .agent
        .command
...
fn resolve_template(root: &Path, local_name: &str, builtin: &str) -> String {
    let local = root.join(".hylomorph").join("prompts").join(local_name);
    if local.exists() {
...
fn cmd_draft_requirements(
    ...
    match load_requirements(&dir) {
        Ok(_) => {
            ...
            println!("Next:  hylomorph spec design {name}");
            Ok(0)
        }
        Err(e) => {
...
fn cmd_draft_design(root: &Path, name: &str) -> Result<i32> {
    ...
    if design_path.exists() {
        ...
        println!("Next:  hylomorph spec tasks {name}");
        Ok(0)
    } else {
...
fn cmd_draft_tasks(root: &Path, name: &str) -> Result<i32> {
    ...
    match validate_spec(root, name) {
        Ok(()) => {
            ...
            println!("Next steps:");
            println!("  hylomorph build {name} --dry-run --once   # preview task selection");
            println!("  hylomorph build {name}                    # generate code");
            Ok(0)
        }
        Err(e) => {
            eprintln!("✗ validation failed: {e:#}");
            eprintln!("\nFix the issues above and re-run:");
            eprintln!("  hylomorph check {name}");
            Ok(1)
        }
    }
}
...
fn cmd_spec_status(root: &Path, name: &str) -> Result<i32> {
    ...
    for layer in Layer::ALL {
        ...
        let detail = match status {
            hylomorph_core::layers::LayerStatus::Invalid(why) => format!("  ({why})"),
            _ => String::new(),
        };
...
fn validate_spec(root: &Path, name: &str) -> Result<()> {
    let dir = spec_dir(root, name);
    let config = load_hylomorph_config(root).unwrap_or_default();
    ...
    let hooks_dir = root.join(".hylomorph").join("scripts").join("hooks");
                ...
                if !known_phases.contains(p.as_str()) {
                    anyhow::bail!(
                        "task {} references phase '{}' which is not defined in \
                         [loop].phase_sequence or [phases.*] in hylomorph.toml",
                        t.id,
                        p
                    );
                }
...
fn detect_cycle(tasks: &[hylomorph_core::spec::Task]) -> Result<()> {
...
fn cmd_gate(root: &Path, cmd: GateCmd) -> Result<i32> {
    match cmd {
        GateCmd::Ls | GateCmd::List => {
            let dir = root.join(".hylomorph").join("scripts").join("hooks");
            if !dir.is_dir() {
...
fn cmd_eval_draft(root: &Path, spec: &str, force: bool, use_code_agent: bool) -> Result<i32> {
    ...
    if !dir.is_dir() {
        anyhow::bail!(
            "no spec '{spec}' at {} — run `hylomorph spec new {spec}` first",
            dir.display()
        );
    }
    ...
    let prompt_path = std::env::temp_dir().join(format!("hylomorph-draft-eval-{spec}.md"));
    std::fs::write(&prompt_path, &prompt)
        .with_context(|| format!("failed to write draft prompt to {}", prompt_path.display()))?;
        ...
        (false, None) => {
            eprintln!(
                "note: no reviewer_command configured — drafting with the primary code agent.\n\
                 For a stronger, independent oracle, set agent.reviewer_command in .hylomorph/hylomorph.toml."
            );
    ...
    println!("  - confirm each stub's assertions encode the requirement's INTENT,");
    println!("    not just whatever the code happens to output;");
    println!("  - confirm no stub depends on src/ at RUNTIME (reading src/ to ground");
    println!("    the invocation is fine; sourcing or importing internals is not);");
    println!("  - resolve any remaining intent `# TODO` (those stubs still exit 1).");
    println!("\nThen check coverage and run them:");
    println!("  hylomorph check {spec}        # every requirement must have an eval");
    println!("  hylomorph eval run {spec}");
    Ok(0)
}

/// Run a spec's eval scripts against the current code. The oracle is invoked
/// the same way the rebuild gate runs it (HYLOMORPH_SPEC / HYLOMORPH_ROOT in env).
fn cmd_eval_run(root: &Path, spec: &str) -> Result<i32> {
    ...
    for ev in &evals {
        ...
        let status = Command::new(ev)
            .current_dir(root)
            .env("HYLOMORPH_SPEC", spec)
            .env("HYLOMORPH_ROOT", root.to_string_lossy().to_string())
            .status();
        match status {
...
fn cmd_explain(
    ...
    let aclc = hylomorph_core::config::resolve_aclc(
        &config,
        &hylomorph_core::config::load_guardrails(root)?,
    );
    let learnings = if config.aclc_present && aclc.memory_on() {
        hylomorph_core::memory::render_for_prompt(&hylomorph_core::memory::load_entries(
...
fn cmd_logs(root: &Path, iteration: Option<u64>, follow: bool) -> Result<i32> {
    let dir = root.join(".hylomorph").join("logs").join("iterations");
    if !dir.is_dir() {
...
fn cmd_aclc(root: &Path, cmd: AclcCmd) -> Result<i32> {
    use hylomorph_core::aclc::{self, Preset};
    use hylomorph_core::config::{load_guardrails, resolve_aclc, save_aclc_config};
    ...
    match cmd {
        ...
        AclcCmd::Preset { name, oracle } => {
            ...
            println!(
                "✓ wrote preset '{}' to .hylomorph/hylomorph.toml",
                preset.name()
            );
            if aclc::has_errors(&findings) {
                println!(
                    "  add an oracle with `hylomorph aclc preset {} --oracle \"<cmd>\"` before running",
                    preset.name()
                );
            }
            Ok(0)
...
fn cmd_doctor(root: &Path) -> Result<i32> {
    ...
    check!(
        ".hylomorph/ exists",
        root.join(".hylomorph").is_dir(),
        "run `hylomorph init`"
    );

    let cfg = load_hylomorph_config(root);
    check!("hylomorph.toml parses", cfg.is_ok(), "fix TOML syntax");
    ...
    if let Ok(c) = &cfg {
        ...
        if !base_cmd.is_empty() && !base_cmd.contains('{') {
            ...
            check!(
                ...
                on_path,
                format!("'{base_cmd}' not found — verify [agent].command in hylomorph.toml")
            );
        }
        ...
        if let Ok(g) = hylomorph_core::config::load_guardrails(root) {
            let aclc = hylomorph_core::config::resolve_aclc(c, &g);
            let findings = hylomorph_core::aclc::validate(&aclc);
            check!(
                "aclc config valid",
                !hylomorph_core::aclc::has_errors(&findings),
                "run `hylomorph aclc validate` for details"
            );
            for f in findings
                .iter()
                .filter(|f| matches!(f.severity, hylomorph_core::aclc::Severity::Warning))
            {
    ...
    if !ok {
        println!("\nFor spec↔code consistency, run `hylomorph check --all`.");
    }
    Ok(if ok { 0 } else { 1 })
}
...
fn cmd_regen(
    ...
    let all_owned = hylomorph_core::manifest::expand_owned_paths(root, owns)?;
    let to_burn: Vec<String> = if let Some(comp_glob) = component {
        let comp_set = hylomorph_core::manifest::build_owns_globset(&[comp_glob.to_string()])?;
        all_owned
            .into_iter()
    ...
    let run_regen = |attempt_label: &str| -> Result<(i32, Vec<String>)> {
        ...
        let untracked_before = hylomorph_core::util::git_list_untracked(root).unwrap_or_default();
        ...
        let (prompt_file, _) = hylomorph_core::prompt::write_prompt_file(&regen_prompt)?;
        ...
        if agent_exit != 0 {
            ...
            let _ = hylomorph_core::util::git_restore_to_head(root, &untracked_before);
            return Ok((agent_exit, vec![]));
        }

        // 4. Run hooks (including evals from evals/<spec>/).
        let guardrails = hylomorph_core::config::load_guardrails(root)?;
        ...
        for hook_name in &hooks_to_run {
            ...
            let timeout = hylomorph_core::hooks::hook_timeout(
                &guardrails,
                hook_name,
                config.hooks.default_timeout_secs,
            );
            let blocking = hylomorph_core::hooks::is_hook_blocking(&guardrails, hook_name);
            match run_hook(root, &inv, &dummy_task, timeout) {
                ...
                Err(e) => {
                    ...
                    if hylomorph_core::hooks::is_hook_blocking(&guardrails, hook_name) {
                        all_passed = false;
                        break;
                    }
                }
        ...
        if reqs.public_interface {
            if let Some(ref reviewer_cmd) = config.agent.reviewer_command {
                if !reviewer_cmd.is_empty() {
                    ...
                    let (review_file, _) =
                        hylomorph_core::prompt::write_prompt_file(&review_prompt)?;
                    let rcmd =
...
fn compose_regen_prompt(
    root: &Path,
    spec_name: &str,
    reqs: &hylomorph_core::spec::RequirementsFile,
    to_burn: &[String],
) -> Result<String> {
...
> implementation — there is none. Do NOT modify .specs/, evals/, or .hylomorph/.**
...
fn cmd_sync_against_code(root: &Path, spec_name: &str) -> Result<()> {
    ...
    let owned_files = hylomorph_core::manifest::expand_owned_paths(root, &reqs.owns)?;
    if owned_files.is_empty() {
        println!("No owned files found on disk for spec '{spec_name}'.");
        return Ok(());
    }

    let out_dir = root.join(".hylomorph").join("roundtrip");
...
"#,
    ...
    let config = load_hylomorph_config(root)?;
    let (prompt_file, _) = hylomorph_core::prompt::write_prompt_file(&prompt)?;
    let cmd_str = config
        .agent
        .command
        .replace("{prompt_file}", &prompt_file.to_string_lossy());
...
const SETUP_AGENT: &str = include_str!("../templates/hylomorph-setup.md");
...
const HYLOMORPH_TOML: &str = r#"[agent]
...
# ID in the manifest and `hylomorph check` warns when it drifts from the baseline.
...
commit_message_template = "hylomorph: {task_id} {task_title}"
...
[prompts]
loop = ".hylomorph/prompts/loop.md"
init = ".hylomorph/prompts/init.md"
...
#
# [phases.plan]
# prompt_template = ".hylomorph/prompts/plan.md"
# hooks            = ["validate_plan"]
#
# [phases.test]
# prompt_template = ".hylomorph/prompts/test.md"
...
# Installed by `hylomorph init` — gates commits on spec↔code consistency.
# Fails if any spec-owned file was hand-edited or the spec changed without rebuild.
hylomorph check --all
"#;

const RUN_GUI_SH: &str = r#"#!/usr/bin/env sh
# Installed by `hylomorph init` — launches the spec-authoring desktop GUI rooted
...
# Prefer an installed `hylomorph-gui` on PATH; otherwise build and run it from a
# Cargo workspace that provides the `hylomorph-gui` package (e.g. when dogfooding
# inside the Hylomorph repo).
if command -v hylomorph-gui >/dev/null 2>&1; then
  exec hylomorph-gui "$@"
else
  exec cargo run -p hylomorph-gui "$@"
fi
"#;
...
deny = [
  ".specs/**",              # spec files are hylomorph-managed; agents must not alter them
  "evals/**",               # eval oracles are human-authored; never agent-writable
  ".hylomorph/manifest.json", # manifest is maintained by the harness
  ".hylomorph/hylomorph.toml",
  ".hylomorph/guardrails/**",
  ".hylomorph/prompts/**",
  ".hylomorph/scripts/**",
  ".git/**",
  "**/secrets*",
...
- **Never modify files under `.hylomorph/`** except appending to
  `.hylomorph/logs/progress.md` as instructed. Do not touch hylomorph.toml,
...
const LOOP_MD: &str = r#"# Hylomorph Loop Prompt

## Constraints
{rules}

> **HARD RULE — DO NOT TOUCH SPEC OR HARNESS FILES**
> You must never create, edit, or delete any file under `.specs/` or `.hylomorph/`
> (except appending a summary line to `.hylomorph/logs/progress.md`).
...
Update .hylomorph/logs/progress.md with a short summary of what you did.
Do not run tests or builds yourself — the harness will validate your work.
"#;

const INIT_MD: &str = r#"# Hylomorph Initializer Prompt

This is the first iteration of a fresh hylomorph run. Set up any environment
...
# Env: HYLOMORPH_HOOK, HYLOMORPH_TASK_ID, HYLOMORPH_SPEC, HYLOMORPH_ITERATION, HYLOMORPH_ATTEMPT, HYLOMORPH_ROOT
