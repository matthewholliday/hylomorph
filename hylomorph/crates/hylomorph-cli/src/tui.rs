//! `hylomorph watch` — a live, read-only terminal dashboard for a Ralph-loop run.
//!
//! The loop persists everything it does to disk as it goes (`.hylomorph/logs/state.json`,
//! `.hylomorph/logs/iterations/*.json`, `.hylomorph/logs/progress.md`, and the per-spec
...
//! alongside `hylomorph run`, or open it after the fact to replay a finished run.
...
use hylomorph_core::snapshot::Snapshot;
use hylomorph_core::spec::{Task, TaskStatus};
use hylomorph_core::state::IterationRecord;
...
mod tests {
    ...
    fn fixture_root(tag: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("hylomorph-tui-test-{}-{tag}", std::process::id()));
        std::fs::remove_dir_all(&dir).ok();
        let logs = dir.join(".hylomorph").join("logs");
    ...
    fn renders_a_frame_with_key_data() {
        ...
        assert!(text.contains("hylomorph"), "header title missing");
