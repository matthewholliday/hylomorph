//! `hylomorph-gui` — a desktop front-end for authoring a spec's five-layer
//! vertical slice.
//!
//! The window is split in two:
//!
//! * **Left column** — every spec under `.specs/`, selectable, with a
//!   *new spec* link just below the list that drafts one from a typed
//!   description or a brief file.
...
//! out to the `hylomorph` CLI (the same command a user would type) and streams its
...
//!
//! Only the **requirements** layer accepts a prompt today (`hylomorph spec
...
use hylomorph_core::layers::{layer_state, Layer, LayerState, LayerStatus};
use hylomorph_core::manifest::expand_owned_paths;
...
struct SettingsDialog {
    ...
    aclc: hylomorph_core::aclc::AclcConfig,
...
impl SettingsDialog {
    fn load(root: &Path) -> SettingsDialog {
        let config = hylomorph_core::config::load_hylomorph_config(root).unwrap_or_default();
        let guardrails = hylomorph_core::config::load_guardrails(root).unwrap_or_default();
        let aclc = hylomorph_core::config::resolve_aclc(&config, &guardrails);
        SettingsDialog {
...
impl GuiApp {
    ...
    fn launch(&mut self, args: Vec<String>) {
        ...
        self.log.clear();
        self.last_exit = None;
        self.log.push(format!("$ hylomorph {}", args.join(" ")));
...
impl GuiApp {
    ...
    fn left_panel(&mut self, ctx: &egui::Context) {
        ...
        egui::SidePanel::left("specs")
            ...
            .show(ctx, |ui| {
                ...
                ui.separator();
                // "settings" stays pinned to the bottom of the column. Global
                // run budgets live in guardrails.toml, not per spec — editable
                // any time, since the loop only reads them at the start of a run.
                ...
                egui::CentralPanel::default().show_inside(ui, |ui| {
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ui.with_layout(egui::Layout::top_down_justified(egui::Align::LEFT), |ui| {
                            ...
                            // "New spec" — right below the list (and still shown
                            // when there are no specs yet, so a fresh project can
                            // bootstrap its first one). Disabled while a job runs,
                            // since creation shells out to a CLI job and only one
                            // can run at a time.
                            ui.add_space(4.0);
                            let running = self.is_running();
    ...
    fn layer_section(&mut self, ui: &mut egui::Ui, layer: Layer) {
        ...
        // On-disk artifact for layers that have an editable file. Present only
        // when the file actually exists, so "Edit" never points at nothing.
        let edit_path = match layer {
            Layer::Requirements => Some("1-requirements.json"),
            Layer::Design => Some("2-design.md"),
            Layer::Tasks => Some("3-tasks.jsonl"),
            Layer::Code | Layer::Evals => None,
        }
        .map(|f| spec_dir(&self.root, &spec).join(f))
        .filter(|p| p.exists());
            ...
            state
                .show_header(ui, |ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        // "Edit" opens the artifact in the OS default app for
                        // its file type. Shown only for layers backed by a file
                        // that exists on disk.
    ...
    /// brief file, run through `hylomorph spec new`.
    fn new_spec_modal(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("New spec")
            ...
            .show(ctx, |ui| {
                ...
                ui.add_space(8.0);
                ui.colored_label(DIM, format!("$ hylomorph {}", new_spec_preview(dialog)));
                ui.add_space(8.0);
                ui.horizontal(|ui| {
                    let create_btn = ui.add_enabled(can_create, egui::Button::new("Create"));
                    if create_btn
                        .on_hover_text(if can_create {
                            "Run `hylomorph spec new` and stream its output below."
                        } else {
    ...
    fn settings_modal(&mut self, ctx: &egui::Context) {
        use hylomorph_core::aclc::{
            self, Learning, LoopMode, Memory, OnExhaustion, Severity, Workspace,
        };
        ...
        egui::Window::new("Settings")
            ...
            .show(ctx, |ui| {
                ...
                ui.label(
                    "Global limits for `hylomorph build`, stored under [budgets] in \
                     .hylomorph/guardrails/guardrails.toml.",
                );
                ui.add_space(8.0);
                egui::Grid::new("settings-grid")
                    ...
                    .spacing([12.0, 10.0])
                    .show(ui, |ui| {
                        ui.label("Max attempts per task (legacy)").on_hover_text(
                            "Legacy per-task attempt cap for `hylomorph build`. Superseded by \
                                 the ACLC max_attempts axis below when ACLC is enabled.",
                        );
                        ui.add(egui::DragValue::new(&mut dialog.max_attempts).range(1..=100))
                            .on_hover_text("Number of attempts allowed per task (1–100).");
                        ui.end_row();

                        ui.label("Max iterations").on_hover_text(
                            "Global ceiling on the total number of loop iterations a single \
                             `hylomorph build` run may perform.",
                        );
                        ui.add(egui::DragValue::new(&mut dialog.max_iterations).range(1..=100_000))
                            .on_hover_text("Total iteration ceiling for a run (1–100000).");
                        ui.end_row();
                    });
                ...
                ui.checkbox(
                    ...
                    "Manage this project's loop with ACLC ([aclc] in hylomorph.toml)",
                )
                .on_hover_text(
                    "When enabled, the agent loop is governed by the orthogonal ACLC axes below \
                     and written to [aclc] in hylomorph.toml. When disabled, the legacy run \
                     budgets apply.",
                );
                ui.colored_label(
                    DIM,
                ...
                ui.add_enabled_ui(dialog.aclc_enabled, |ui| {
                    ...
                    egui::Grid::new("aclc-grid")
                        ...
                        .show(ui, |ui| {
                            ...
                            egui::ComboBox::from_id_salt("aclc-loop")
                                ...
                                .show_ui(ui, |ui| {
                                    ui.selectable_value(&mut a.loop_mode, LoopMode::Off, "off")
                                        .on_hover_text(
                                            "One attempt, end to end. The harness returns the \
                                             workspace regardless of the oracle result.",
                                        );
                                    ui.selectable_value(
                                        &mut a.loop_mode,
                                        LoopMode::UntilPass,
                            ...
                            ui.add_enabled_ui(looping, |ui| {
                                ui.label("oracle.command").on_hover_text(
                                    "Shell command whose exit status decides pass/fail. \
                                     Required when loop = until_pass; inert otherwise.",
                                )
                            });
                            ui.add_enabled_ui(looping, |ui| {
                                ...
                                let resp = ui
                                    .add(
                                        egui::TextEdit::singleline(&mut cmd)
                                            .hint_text("e.g. hylomorph eval run <spec>")
                                            .desired_width(220.0),
                                    )
                                    .on_hover_text(
                                        "The exit status of this command decides pass/fail for \
                                         each attempt. Leave empty to use the spec's protected \
                                         eval suite.",
                                    );
                                if resp.changed() {
        ...
        if save {
            let mut result = hylomorph_core::config::save_guardrail_budgets(
                &root,
                dialog.max_attempts,
                dialog.max_iterations,
            );
            if result.is_ok() && dialog.aclc_enabled {
                result = hylomorph_core::config::save_aclc_config(&root, &dialog.aclc);
            }
            match result {
                Ok(()) => done = true,
...
/// Open `path` in the OS default application for its file type. Fire-and-forget:
/// the launcher is detached and any failure is ignored, matching how a desktop
/// "open in app" affordance behaves.
